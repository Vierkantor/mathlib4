...
* `nhdsKer s`: The _neighborhoods kernel_ of a set is the intersection of all its neighborhoods.
...
* `ClusterPt x F`: a point `x` is a _cluster point_ of a filter `F`,
  if `𝓝 x` is not disjoint with `F`.
* `MapClusterPt x F u`: a point `x` is a _cluster point_ of a function `u` along a filter `F`,
  if it is a cluster point of the filter `Filter.map u F`.
* `AccPt x F`: a point `x` is an _accumulation point_ of a filter `F`,
  if `𝓝[≠] x` is not disjoint with `F`.
...
/--
The _neighborhoods kernel_ of a set is the intersection of all its neighborhoods. In an
...
/--
Two points `x` and `y` in a topological space are `Inseparable`, or _topologically
indistinguishable_, if any of the following equivalent properties hold:
...
This is _not_ the same as asking `𝓝[≠] x ⊓ F ≠ ⊥`, which is called `AccPt` in Mathlib.
...
/--
We say that a topological space is a _weakly locally compact space_,
