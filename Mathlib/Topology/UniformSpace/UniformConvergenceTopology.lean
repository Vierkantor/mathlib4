...
`UniformOnFun α β 𝔖` (denoted `α →ᵤ[𝔖] β`) and we actually endow _these_ with the structures
...
* Show that the uniform structure of `𝔖`-convergence is exactly the structure of `𝔖'`-convergence,
  where `𝔖'` is the _*noncovering*_ bornology (i.e _*not*_ what `Bornology` currently refers
...
_*noncovering*_ bornology generated by `𝔖`.
-/
