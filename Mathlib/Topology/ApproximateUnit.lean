...
An _approximate unit_ is a filter `l` such that multiplication on the left (or right) by `m : α`
...
/--
An _approximate unit_ is a proper filter (i.e., `≠ ⊥`) such that multiplication on the left
