...
Likewise, `MonoidWithZeroHom.valueGroup` is the _subgroup_ of `Bˣ` generated by the invertible
...
* When `B` is a _commutative_ group with zero, `MonoidWithZeroHom.valueGroup` can be
...
/--
When the _domain_ is itself a group with zero, the `valueMonoid` and the `valueGroup` coincide.
-/
