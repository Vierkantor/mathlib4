...
literature (see the seminal paper \[_The positive cone in Banach algebras_\]\[kelleyVaught1953\])
...
it usually does _not_ satisfy `IsOrderedRing`.
...
* In a Banach star algebra without a well-defined square root, the natural ordering is given by the
  ...
  ‍\[_The positive cone in Banach algebras_\]\[kelleyVaught1953\]). Note that the current definition
  has
