...
Let $`K` be the field of fractions of a Dedekind domain $`R`. For any set $`S` of prime ideals in
the
height one spectrum of $`R`, and for any natural number $`n`, the Selmer group $`K(S, n)` is defined
to
be the subgroup of the unit group $`K^\times` modulo $`n`-th powers where each element has $`v`-adic
valuation divisible by $`n` for all prime ideals $`v` away from $`S`. In other words, this is
precisely
...
where $`R_S^\times` is the $`S`-unit group of $`R` and $`\mathrm{Cl}_S(R)` is the $`S`-class group
of
$`R`. If the flanking groups are both finite, then $`K(S, n)` is finite by the first isomorphism
theorem. Such is the case when $`R` is the ring of integers of a number field $`K`, $`S` is finite,
and
...
with certain local conditions defined by $`v`-adic valuations, where $`\mu_n` is the group of
$`n`-th
...
https://doc.sagemath.org/html/en/reference/number\_fields/sage/rings/number\_field/selmer\_group.html
