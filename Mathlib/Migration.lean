...
* `{url}` turns a code literal containing a URL into a link.
* `{library_note}` references a library note and checks that it exists.
* `{cite}` references a bibliography entry and renders custom text for it.
* The `code` code block records a language for a verbatim code listing.
...
/-- A reference to a bibliography entry in `docs/references.bib`. -/
structure Citation where
  /-- The citation key, as it appears in `docs/references.bib`. -/
  key : String
...
/--
Extracts the single code literal from the contents of a role, such as the `` `x` `` in `` {role}`x` ``.
-/
...
/-- Whether `docs/references.bib` contains an entry with the given key. -/
...
The code literal contains the target, which is also used as the link text, as in
`` {url}`https://leanprover.org` ``.
-/
@[doc_role]
...
The positional argument is the entry's key in `docs/references.bib`, and the bracketed content is the
text to render, as in `` {cite "Adamek_Rosicky_1994"}[Adámek–Rosický] ``. A warning is emitted when
the key is not found in `docs/references.bib`.
-/
@[doc_role]
