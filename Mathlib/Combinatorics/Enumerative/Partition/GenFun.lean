...
* When $`f(i, c) = 1` for odd $`i` and $`f(i, c) = 0` for even $`i`, this is the generating function
  for
...
part of the partition, and $`c` is the count of that part in the partition. The character function
is
