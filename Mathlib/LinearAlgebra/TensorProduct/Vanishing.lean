...
Let $`M` and $`N` be modules over a commutative ring $`R`. Recall that every element of
$`M \otimes N`
...
_vanishes trivially_ (`TensorProduct.VanishesTrivially`) if there exist a finite index type
...
The _equational criterion for vanishing_ (`TensorProduct.vanishesTrivially_iff_sum_tmul_eq_zero`),
which appears as
‍\[A. Altman and S. Kleiman, _A term of commutative algebra_ (Lemma 8.16)\]\[altman2021term\],
states that if the elements $`m_i` generate the module $`M`, then $`\sum_i m_i \otimes n_i = 0` if
and
...
_equational criterion for flatness_ `Module.Flat.iff_forall_isTrivialRelation`.)
...
_vanishes trivially_ if there exist a finite index type $`\kappa` = `Fin k`,
elements $`(y_j)_{j \in \kappa}` of $`N`, and elements $`(a_{ij})_{i \in \iota, j \in \kappa}` of
$`R`
such that for all $`i`,
$$`n_i = \sum_j a_{ij} y_j`
and for all $`j`,
