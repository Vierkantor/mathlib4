...
A linear endomorphism `f` satisfying these equivalent conditions is known as a _semisimple_
...
/--
A linear endomorphism of an `R`-module `M` is called _semisimple_ if the induced `R[X]`-module
