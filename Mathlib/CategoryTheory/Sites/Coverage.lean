...
The main difference between a coverage and a Grothendieck pretopology is that we _do not_
...
* [nLab, _Coverage_](https://ncatlab.org/nlab/show/coverage)
-/
...
we say that _`S` factors through `T` along `f`_, written `S.FactorsThruAlong T f`,
...
The lemma `Presieve.isSheafFor_of_factorsThru` gives a _sufficient_ condition for a
...
A coverage is a collection of _covering_ presieves on every object `X : C`,
which satisfies a _pullback compatibility_ condition.
...
It is defined _inductively_ as follows:
...
3. Add all sieves required by the _local character_ axiom of a Grothendieck topology.
