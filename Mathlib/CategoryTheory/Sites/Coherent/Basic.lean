...
The first one is called the _coherent_ topology. For that to exist, the category `C` must satisfy a
...
The second one is called the _regular_ topology and for that to exist, the category `C` must satisfy
...
The third one is called the _extensive_ coverage and for that to exist, the category `C` must
...
* [nLab, _Coherent Coverage_](https://ncatlab.org/nlab/show/coherent+coverage)
-/
