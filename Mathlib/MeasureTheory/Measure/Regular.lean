...
‍\[Fremlin, _Measure Theory_ (volume 4, 411J)\]\[fremlin\_vol4\] considers measures which are inner
regular with respect to compact closed sets (and calls them _tight_). However, since most of the
...
‍\[Halmos, Measure Theory, §52\]\[halmos1950measure\]. Note that Halmos uses an unusual definition
of
...
/--
We say that a measure `μ` is _inner regular_ with respect to predicates `p q : Set α → Prop`,
