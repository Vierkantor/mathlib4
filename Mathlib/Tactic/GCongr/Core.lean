...
limited amount of forward reasoning.  Such attempts are made _before_ descending further into
...
then a direct consequence like `a ≤ b → a + c ≤ b + c` does _not_ need to be tagged.
