...
these are a footgun, as the attribute is applied _globally_ (despite the `in`).
-/
...
Despite the `in`, these define _global_ instances, which can be rather misleading.
