...
* [https://en.wikipedia.org/wiki/Monge\_point](https://en.wikipedia.org/wiki/Monge_point)
* [
  https://en.wikipedia.org/wiki/Orthocentric\_system](https://en.wikipedia.org/wiki/Orthocentric_system)
...
either we can find indices `i₁`, `i₂` and `i₃` for `p` such that `p i₁` is the orthocenter of `t`
and `p i₂` and `p i₃` are points `j₂`
