...
* `bDistortion`: the value `true` means that `r` can depend on the maximal ratio of sides of the
  ...
  `BoxIntegral.IntegrationParams.GP = ⊥ = {bRiemann := false, bHenstock := true, bDistortion := true}`.
...
* `BoxIntegral.IntegrationParams.Henstock` (`bRiemann = false`, `bHenstock = true`,
  ...
  of a partition, we require that the partition is _subordinate_ to a possibly discontinuous
...
the divergence theorem only for the filter
`BoxIntegral.IntegrationParams.GP = ⊥ = {bRiemann := false, bHenstock := true, bDistortion := true}`.
  -/
  (bDistortion : Bool)
