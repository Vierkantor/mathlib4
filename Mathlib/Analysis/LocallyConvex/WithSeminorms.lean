...
* `PolynormableSpace 𝕜 E` is a class asserting that the (existing) topology on `E` is induced
  by _some_ family of `𝕜`-seminorms. If `𝕜` is `RCLike`, this is equivalent to
  `LocallyConvexSpace 𝕜 E`.
  The terminology is inspired by N. Bourbaki, _Variétés différentielles et analytiques_. However,
...
_some_ family of `𝕜`-seminorms. Equivalently, its topology is induced by _all_ its continuous
seminorm.
...
then it is uniformly controlled by _finitely many_ seminorms of `p`, that is there
