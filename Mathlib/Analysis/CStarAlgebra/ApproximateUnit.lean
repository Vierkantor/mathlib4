...
* `CFC.monotoneOn_one_sub_one_add_inv`: the function `f := fun x : ℝ≥0, 1 - (1 + x)⁻¹` is
  _operator monotone_ on `Set.Ici (0 : A)` (i.e., `cfcₙ f` is monotone on `{x : A | 0 ≤ x}`).
...
* `Filter.IsIncreasingApproximateUnit`: a filter `l` is an _increasing approximate unit_ if it is an
...
/--
An _increasing approximate unit_ in a C⋆-algebra is an approximate unit contained in the
