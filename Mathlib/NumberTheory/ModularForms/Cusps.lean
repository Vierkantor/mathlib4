...
/--
The _cusps_ of a subgroup of `GL(2, ℝ)` are the fixed points of parabolic elements of `g`.
-/
...
We define the _strict width_ of `𝒢` at `∞` to be the smallest `h > 0` such that `[1, h; 0, 1] ∈ 𝒢`,
or `0` if no such `h` exists; and the _width_ of `𝒢` to be the strict width of the subgroup
...
/--
A subgroup is _regular at ∞_ if its periods and strict periods coincide.
-/
