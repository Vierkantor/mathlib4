...
The proof is loosely based on the strategy given in \[D. Marcus, _Number
Fields_\]\[marcus1977number\].
...
6. We define a set `paramSet` in `realSpace K` and prove that
   ...
   `normAtAllPlaces_normLeOne_eq_image`. Using this, `setLIntegral_expMapBasis_image` and the
   results
...
7.4. Finally, we prove that
`volume (normAtAllPlaces ⁻¹' compactSet K) = volume (normAtAllPlaces ⁻¹' (expMapBasis '' interior (paramSet K)))`,
which implies that
